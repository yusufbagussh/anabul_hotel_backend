...
func (db *serviceConnection) GetAllService(hotelID string, filterPagination dto.FilterPagination) ([]entity.Service, dto.Pagination, error) {
	...
	query := db.connection.Model(&services)
	...
	query.Where(whereClause).Scopes(func(db *gorm.DB) *gorm.DB {
		if filterPagination.HotelID != "" {
			db.Where("services.hotel_id = ?", filterPagination.HotelID)
		} else {
			db.Where("services.hotel_id = ?", hotelID)
		}
		return db
	})
	...
	err := query.Count(&total).Limit(perPage).Offset((page - 1) * perPage).Preload("Hotel").Find(&services).Error
